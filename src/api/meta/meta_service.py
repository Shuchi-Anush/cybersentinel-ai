...
class MetaService:
    ...
    def __init__(self) -> None:
        ...
        # Evaluation source (metrics.json is the artifact generated by pipeline)
        from src.core.paths import EVAL_DIR
        self._eval_summary = self._load_json_optional(EVAL_DIR / "metrics.json")
    ...
    def get_eval(self) -> dict:
        """
        Evaluation summary with status tracking. 
        Returns fallback if evaluation has not been run or fails.
        """
        try:
            if not self._eval_summary:
                return {"f1_macro": 0.0, "status": "missing"}
            
            # Enforce contract field from full eval summary
            return {
                "f1_macro": float(self._eval_summary.get("multiclass", {}).get("f1_macro", 0.0)),
                "status": "ok"
            }
        except Exception as e:
            logger.error(f"MetaService: Unexpected error in get_eval: {e}")
            return {"f1_macro": 0.0, "status": "missing"}
    ...
    def _load_json(path: Path) -> dict:
        ...
        if not path.exists():
            logger.warning("MetaService: required file not found: %s", path)
            return {}
        try:
            with open(path, "r") as fh:
                return json.load(fh)
        except Exception as e:
            logger.error("MetaService: failed to parse JSON at %s: %s", path, e)
            return {}

    @staticmethod
    def _load_yaml(path: Path) -> dict:
        """Load a YAML file defensively."""
        if not path.exists():
            logger.warning("MetaService: required file not found: %s", path)
            return {}
        try:
            ...
            logger.error("MetaService: failed to parse YAML at %s: %s", path, e)
            return {}

    @staticmethod
    def _load_json_optional(path: Path) -> dict:
        """Load an optional JSON file defensively."""
        if not path.exists():
            return {}
        try:
            with open(path, "r") as fh:
